...
double ans = 1e18;
bool chk(double mid, vector<pair<ll, ll>> xv)
{
    ll n = xv.size();
    double lp = 0, rp = 0;
    FOR(i, 0, n)
    {
        double xi = xv[i].first, vi = xv[i].second;
        if (xi < mid)
        {
            lp = max(lp, double(abs(xi - mid) / vi));
        }
        else
        {
            rp = max(rp, double(abs(xi - mid) / vi));
        }
    }
    ans = max(lp, rp);
