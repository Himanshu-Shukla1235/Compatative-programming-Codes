...
bool canFit(ll s, ll w, ll h, ll n) {
    ll rectanglesInWidth = s / w;
    ll rectanglesInHeight = s / h;

    
    if (rectanglesInWidth >= n || rectanglesInHeight >= n) return true;
    return (rectanglesInWidth >= n / rectanglesInHeight) && (rectanglesInWidth * rectanglesInHeight >= n);
}
