...
void logic()
{
    ...
    // Create a sieve to mark primes
    vector<bool> isPrime(1e5 + 1, true);
    isPrime[0] = isPrime[1] = false; // 0 and 1 are not prime numbers
    vector<vector<ll>> pr(1e5 + 1);

    for (ll i = 2; i <= 1e5; i++)
    {
        if (isPrime[i])
        {
            pr[i].push_back(i); // Prime number's only factor is itself
            for (ll j = 2 * i; j <= 1e5; j += i)
            {
                isPrime[j] = false;
                pr[j].push_back(i); // Store the prime factor
            }
        }
    }

    map<ll, ll> mp1; // To track which numbers are turned "on"
    map<ll, ll> mp2; // To track prime factors that are already used

    vector<pair<char, ll>> q(m + 1); // To store the operations (+ or -) and the number

    // Read the operations
    cin.ignore(); // To ignore the newline after reading m
    for (auto &val : q)
    {
        string s;
        getline(cin, s);
        ...
        if (s.size() > 2)
        {
            ll num = stoll(s.substr(2)); // Extract number from string
            char sign = s[0];            // Get the sign

            val.first = sign; // Assign sign to the first element
            val.second = num; // Assign the number to the second element
        }
    }

    // Output each pair in the vector
    // for (const auto& val : q) {
    //     cout << "Sign: " << val.first << ", Number: " << val.second << endl;
    // }
    ...
    for (ll i = 0; i < m; i++)
    {
        ...
        if (q[i].first == '+')
        {
            ...
            else
            {
                ...
                for (ll prime : pr[num])
                {
                    if (mp2[prime] != 0)
                    {
                        cout << "Conflict with" << " " << mp2[prime] << endl;
                        ch = false;
                        break;
                    }
                }
                if (ch)
                {
                    ...
                    for (ll prime : pr[num])
                    {
                            ...
                            mp2[prime] = num; // Mark prime factor as used
        ...
        else
        {
            ...
            {
                cout << "Success "  << endl;
                mp1[num] = 0; // Turn the number "off"
                for (ll prime : pr[num])
                {

                    {
                        mp2[prime] = 0; // Mark prime factor as used
                    }
                }
            }
        }
