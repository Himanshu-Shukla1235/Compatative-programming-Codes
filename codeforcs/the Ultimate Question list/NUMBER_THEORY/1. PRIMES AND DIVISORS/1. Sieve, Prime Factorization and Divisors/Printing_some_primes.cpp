...
// note : in this n= 1e8 is acceptible thats why i use sieve but in codeforces for large may be this not work
void logic()
{
    ll n = 1e8;
    vector<bool> v(n + 1, false); // Use a boolean vector to save memory
    ...
    for (ll i = 2; i * i <= n; i++)
    {
        if (!v[i])
        {
            ...
            {
                v[j] = true; // Mark multiples of i as non-prime
            }
        }
    }
