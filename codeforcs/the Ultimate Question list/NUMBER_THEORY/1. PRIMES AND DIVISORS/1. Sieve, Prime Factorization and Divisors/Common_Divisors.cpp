...
void logic()
{
    ...
    // Iterate from largest possible GCD downwards
    for (ll i = maxelem; i >= 2; i--)
    {
        ll count = 0;

        // Count multiples of i
        for (ll j = i; j <= maxelem; j += i)
        {
            count += freq[j];
        }

        // If at least two numbers have this GCD
        if (count >= 2)
        {
            COUT(i);
            return;
    ...
    // If no GCD is found, it defaults to 1
    COUT(1);
}
//logic-2
