//*______________________________________________ 1000 || Monsters __________________________________________________
/*
                                                                                                                                                                       //|
                                                                                                                                                                       //|
*/
...
void logic()
{
    ...
    // Handle the edge case where k == 1
    if (k == 1)
    {
        // Since every element % 1 will be 0, just print the indices in the original order
        for (int i = 0; i < n; i++)
        {
            cout << p[i].second << " ";
        }
        COUT(""); // End of output
        return; // Return here since we've handled the k = 1 case
    }

    // For k > 1, perform the usual modulo operation
    for (int i = 0; i < n; i++)
    {
        if (k <= p[i].first)
        {
    ...
    // Sort based on the first value of the pair
    sort(p.begin(), p.end());

    // Print indices where first == 0
    for (int i = 0; i < n; i++)
    {
        if (p[i].first == 0)
        {
