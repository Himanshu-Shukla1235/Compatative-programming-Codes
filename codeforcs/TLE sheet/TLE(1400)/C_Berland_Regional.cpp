...
void logic()
{
    ...
    for (auto &entry : mp)
    {
        ...
        for (ll i = 1; i < entry.second.size(); i++)
        {
            entry.second[i] += entry.second[i - 1];
        }
    }
...
// Concept of efficient sol. : resuce the time complexity but the ologic is same       ......

// concpt used by me : same but time commplexity incresase cause the way i implement

// suggestion : focus on the time complexity;
