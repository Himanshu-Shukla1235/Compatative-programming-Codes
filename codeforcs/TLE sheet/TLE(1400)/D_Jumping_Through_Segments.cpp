//*______________________________________________ D_Jumping_Through_Segments || 1400 __________________________________________________
/*
                                                                                                                                                                       //|
                                                                                                                                                                       //|
*/
...
bool canjump(ll n, vector<pair<ll, ll>> p, ll k)
{
    ll ljump = 0, rjump = k;

    for (ll i = 0; i < n; ++i)
    {
        // If the current interval is outside the current jump range
        if (p[i].second < ljump || p[i].first > rjump)
        {
            return false;
        }

        // Update the current jump range
        ljump = max(p[i].first, ljump);
        rjump = min(p[i].second, rjump);

        // Adjust for the next jump
        ljump = max(0LL, ljump - k);
        rjump = rjump + k;
    }
...
void logic()
{
    ...
    ll l = 0, h = 1e9, result = 0;
    while (l <= h)
    {
        ll mid = (l + h) / 2;
        bool check = canjump(n, v, mid);
        if (check)
        {
            result = mid;
            h = mid-1; // Srink the upper bound to mid, to find the maximum satisfying value
        }
        else
        {
            l = mid + 1; // Increase the lower bound
        }
    }
...
// Concept of efficient sol. : binary search use  ......

// concpt used by me : just a range logic 

// suggestion : try more binary search question ............
