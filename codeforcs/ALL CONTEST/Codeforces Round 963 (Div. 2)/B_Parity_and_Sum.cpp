...
void logic()
{
    ...
    // Read values and separate odd values into v1
    AUTO_IT(v)
    {
        cin >> val;
        if (IS_ODD(val))
    ...
    // Set maxodd to the largest odd value
    ll maxodd = v1.back();
    ...
    FOR(i, n)
    {
        ...
        if (IS_EVEN(v[i]))
        {
            // Update maxodd if the current even value is combined with the largest odd
            // if (i != n - 1)
            // {
            //     maxodd = max(maxodd, v[i] + maxodd);
            // }
            result++;
        }
    }
