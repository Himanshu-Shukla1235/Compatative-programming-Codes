...
vector<int> findUnsetBitIndices(int a)
{
    vector<int> unsetBitIndices;
    int index = 0;

    while (a > 0 || index < 32) // assuming 32-bit integer
    {
        if ((a & 1) == 0) // check if the lowest bit is not set
        {
            unsetBitIndices.push_back(index);
        }
        a >>= 1; // shift right to check the next bit
        index++;
    }

    // Reverse the indices to start from the last
    reverse(unsetBitIndices.begin(), unsetBitIndices.end());

    return unsetBitIndices;
}
int main()
{
...
void logic()
{
    ...
    v[1] = n;
    set<ll> ods;
    vector<int> unsetind = findUnsetBitIndices(n);
    FOR(i, 0, unsetind.size())
    {
        if ((n + pow(2, unsetind[i])) < n)
        {
            ods.insert()
        }
    }
