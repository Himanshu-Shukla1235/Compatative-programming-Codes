...
void logic()
{
    ...
    ll j = 0; // Pointer for iterating over string s
    ...
    FOR(i, t.size())
    {
        ...
        for (; j < s.size(); j++)
        {
            if (t[i] == s[j])
            {
                // Found a matching character in s
                check = true;
            ...
            {
                // Replace '?' with the current character of t
                s[j] = t[i];
                check = true;
                j++; // Move to the next character in s
                break;
        ...
        {
            COUT("NO");
            return; // Exit function if no match found
        }
    }
    FOR(i, s.size())
    {
