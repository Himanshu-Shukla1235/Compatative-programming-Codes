...
void logic()
{
    ...
    // Initialize a vector large enough for the maximum possible index.
    vector<ll> v(n + 1, 0);
    vector<ll> v2 = {1, 2, 2, 3, 3, 4, 4, 5, 5, 1, 6, 7, 7, 8, 8, 9, 9, 10, 10, 11, 11, 12, 12, 13, 13, 1, 6};
    ...
    if (n % 2 == 0)
    {
        // Filling the vector v for even n
        FOR(i, 1, n)
        {
            if (cnt >= n)
                break;
    ...
    {
        // Ensure v2 has exactly n elements
        ...
        // Print the first n elements of v2
        FOR(i, 0, n)
        {
            cout << v2[i] << " ";
        }
    ...
    // Print the vector v up to `n` elements
    FOR(i, 0, n)
    {
        cout << v[i] << " ";
    }
