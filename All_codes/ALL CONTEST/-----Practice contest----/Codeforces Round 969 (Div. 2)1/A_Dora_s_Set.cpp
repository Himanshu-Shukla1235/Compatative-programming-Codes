...
void logic()
{
    ...
    set<ll> s; // Set to keep track of used numbers
    ll result = 0;
    ...
    for (ll i = x; i < y; i++)
    {
        ...
        for (ll j = x; j < y; j++)
        {
            if (s.find(j) != s.end() || j == i)
            {
                continue; // Skip if already processed or if i == j
            }

            for (ll k = x; k < y; k++)
            {
                if (s.find(k) != s.end() || k == i || k == j)
                {
                    continue; // Skip if already processed or if k equals i or j
                }
                ...
                {
                    result++;
                    // Add i, j, k to the set
                    s.insert(i);
                    s.insert(j);
                    s.insert(k);
                    break; // Break inner loop after finding valid triplet
                }
            }
            break; // Break middle loop after valid triplet
        }
    }
