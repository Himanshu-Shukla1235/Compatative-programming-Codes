...
void logic()
{
    ...
    // long long number = 1234567890; // Example number
    vector<int> digits;
    ...
    for (char c : numStr)
    {
        // Convert character to integer and store in the vector
    ...
    if (digits[0] != 1 || digits[1] != 0 || ((digits[2] * 10) + digits[3]) < 2 || digits[2] == 0)
    {
        COUT("NO");
        return;
    }
