...
void logic() {
    ...
    cin >> n;
    vector<ll> v(n);
    vector<ll> v1(n, 0);  // Initialize with 0
    ...
    // Initialize vector v1 with values from 1 to n
    for (ll i = 0; i < n; ++i) {
        v1[i] = i + 1;
    }
    ...
    for (ll i = 0; i < n; ++i) {
        ...
        // Handle edge cases
        if (v[i] == 1) {
            if (n > 1) v1[v[i]] = 0; // Ensure we don't access out-of-bounds
        } else if (v[i] == n) {
            if (n > 1) v1[v[i] - 2] = 0; // Ensure we don't access out-of-bounds
        } else {
            if (v1[v[i] - 2] != -1) {
                v1[v[i] - 2] = 0;
            }
