...
void logic() {
    ...
    while (z--) {
        string s;
        map<char, ll> charToVal; // Mapping from characters to values
        map<ll, char> valToChar; // Mapping from values to characters

        cin >> s;

        bool flag = false;

        // Check if the size of s matches n
        if (s.size() != n) {
            cout << "NO" << endl;
            continue;
        }
        ...
        // Validate the character-to-value mapping
        for (ll i = 0; i < n; ++i) {
            if (charToVal[s[i]] != v[i]) {
                cout << "NO" << endl;
                flag = true;
                break;
            }
        ...
        // Validate the value-to-character mapping
        if (!flag) {
            for (ll i = 0; i < n; ++i) {
                if (valToChar[v[i]] != s[i]) {
                    cout << "NO" << endl;
                    flag = true;
                    break;
                }
