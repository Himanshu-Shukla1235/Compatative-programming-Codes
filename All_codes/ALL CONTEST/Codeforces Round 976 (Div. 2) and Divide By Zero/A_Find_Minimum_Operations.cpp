...
long long log_base(long a, long b) {
    // Using the change of base formula
    return static_cast<long long>(log(a) / log(b)); // Change of base formula
}

// Function to perform the logic based on n and k
void logic() {
    long long n, k; // Using long long for large values
    cin >> n >> k;

    // If n is less than or equal to k, return n - k
    if (n <= k) {
        cout << (n - k) << endl; // Use cout instead of COUT
        return;
    }

    // Calculate the result using the log_base function
    long long result = n - log_base(k, n) + 1;
    cout << result << endl; // Use cout instead of COUT
}
