...
void logic()
{
    ...
    while (sum < 1e12)
    {
        sum = (1LL << k); // Use bitwise shift to avoid precision issues
        k++;
 ...
 // Ensure last segment is considered

    // auto f = lower_bound(pre.begin(), pre.end(), re);
    // if (f == pre.end()) re = pre.size(); // Prevent out-of-bounds issue
    // else re = f - pre.begin();

    COUT(ceil((double)log2(cnt + 1)));
}
