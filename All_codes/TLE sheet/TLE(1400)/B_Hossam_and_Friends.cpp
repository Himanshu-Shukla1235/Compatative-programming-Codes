//*______________________________________________ 1400 || Hossam and Friends __________________________________________________
/*
                                                                                                                                                                       //|
                                                                                                                                                                       //|
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>__           __<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
void logic()
{
    ll n, m;
    cin >> n >> m;
    VEC_PAIR(int, int, p, m);
    std::set<ll> en; // Use set instead of vector

    AUTO_IT(p)
    {
        cin >> val.first >> val.second;
        en.insert(val.second); // Insert second element of pair into set
    }
    ...
    for (ll i = 1; i <= n - 1; i++)
    {
        ...
        if (i < *it) // Compare current 'i' with the value pointed by the iterator
        {
            re += (*it - i - 1);
        }
        else
        {
            ...
            // Check again if 'it' is valid before accessing it
            if (it != en.end())
            {
                re += (*it - i - 1);
            }
            ...
            {
                // Iterator reached the end, handle the remaining elements
                re += (n - i);
                // cout << n - i << ";
            }
        }
...
void logic2()
{
    ...
    vector<ll> v(n + 1, -1); // Initialize with empty strings
