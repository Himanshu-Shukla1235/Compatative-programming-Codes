//*______________________________________________ 1900 || Dances(hard verrsion) __________________________________________________
/*
                                                                                                                                                                       //|
                                                                                                                                                                       //|
*/
...
ll opera(vector<ll> a, vector<ll> b, ll m, ll n)
{
    a[0] = m;
    ll result = 0;
    ll ai = n - 1;

    for (ll i = n - 1; i >= result; i--)
    {
        while (b[i] <= a[ai] && ai >= 0)
        {
...
void logic()
{
    ...
    // Input for vector a
    FOR(i, 0, n) // Corrected to zero-based index
    {
        cin >> a[i];
    }
    ...
    ll re = opera(a, b, 1, n); // Initial operation result
   
 for (int i = 0;index!=c.end() ; i++)
 {
    while
 }
    ...
    // Loop through each possible value of 'i' up to m
    bool check=false;
   
    
    
    COUT(re); // Print the final result
}
